...
items.addMany = function(user, manyItems, done) {
  async.forEach(manyItems, function(item, cb) { return items.add(user, item, cb); }, done);
};

items.add = function(user, item, done) {
  item.day = moment(item.timestamp).sod().valueOf();

  async.series([
    function(cb) { items.addTags(user, item, cb); },
...
items.addTags = function(userid, item, done) {
  return tags.addMany(userid, item.tags, done);
};
