...
items._add = function(user, item, done) {
  async.series([
    function(cb) { items._addStartOfDay(user, item, cb); },
...
items._aggregateByDay = function(userid, item, done) {
   async.waterfall([
    ...
    function(collection, cb) {
      ...
      return collection.update(byDay, { "$inc" : { sumProcedures: 1, sumRatings: item.rating, sumFailures: item.attempts - 1 } }, { upsert: true }, cb);
    }
  ], done);
};
